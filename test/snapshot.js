describe('snapshot', () => {
  ...
  const defaultCondition = (window, customSettings, src) => {
    ...
    return window.htmlPdfClient().set(settings).from(src || window.document.body).outputPdf('arraybuffer');
  };
  ...
  const conditions = {
    ...
    legacy: {
      runner: window => window.htmlPdfClient(window.document.body, defaultSettings).outputPdf('arraybuffer'),
      name: file => `${file}.pdf`,
    },
    margin: {
  ...
  Object.keys(filesToTest).forEach(file => describe(file, () => {
    ...
    before(async (done) => {
      ...
      chai.spy.on(iframe.contentWindow.htmlPdfClient.Worker.prototype, 'save', function () { return this.then(function save() {}); });
       done();
    });
    after((done) => {
      chai.spy.restore();
    ...
    filesToTest[file].forEach(condition => it(`should match snapshot for ${condition} settings`, async (done) => {
